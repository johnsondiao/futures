...
方法论与 5m 版一致（见 scripts/goal_search_full.py + robustness_report.py）：
  不挑单点最优，找「入场逻辑固定后，ATR 退出怎么调都大概率盈利」的稳健区域。

用法:
  python scripts/search_60m.py [--top 12]
"""
...
def main():
    ...
    if not cache.exists():
        raise SystemExit(f"缺少缓存 {cache}，先跑 scripts/run_backtest.py --period 60m")
    df = load_cache(cache)
