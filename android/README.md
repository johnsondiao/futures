...
- 首次进入盘面时会请求通知权限；之后 K 线**新出现**「开多/开空」标记时，会播放提示音并弹出系统通知（首次加载历史信号不打扰）。可在 App「设置」中开关声音/震动/通知/横幅。
- 顶部「介绍」可查看带示意图的策略说明。
- 若试听无系统通知：到手机系统设置允许本应用通知，并确认通知类别「开仓信号提醒」为开启/高重要性。提示音与震动不依赖通知权限。
...
默认合约：`DCE.a2611`（可在「设置 → 交易品种」修改），周期：5 分钟。
